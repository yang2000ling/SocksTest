...
def nodes_deduplication(nodes_list, nodes_path=NODES_TXT_PATH):
    ...
    for i in data_list:
        if len(i) >= 10:
            f_nodes_w.write(i + '\n')
    print('原有{}节点，现有{}节点，增加{}节点！'.format(data_list_len, len(data_list), len(data_list) - data_list_len))


def sub_to_data(sub_buff):
    """读取订阅内容sub_text，节点去重后写入nodes.txt"""
    nodes_list = node_lib.subscribe_to_list(sub_buff)
    nodes_deduplication(nodes_list)
